...
class DoubleSliderEntry extends TooltipListEntry<Double> {
    ...
    public void render(PoseStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
        ...
        if (MC.font.isBidirectional()) {
            drawString(matrices, MC.font, name, window.getGuiScaledWidth() - x - MC.font.width(name), y + 6, getPreferredTextColor());
            resetButton.x = x;
            sliderWidget.x = x + resetButton.getWidth() + 1;
        } else {
            drawString(matrices, MC.font, name, x, y + 6, getPreferredTextColor());
