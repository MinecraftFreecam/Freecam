        ...
        registry.registerPredicateTransformer(
                (guis, i18n, field, config, defaults, guiProvider) -> {
                    ignoreCustom = guis.stream()
                            ...
                            .reduce((prev, next) -> { throw new IllegalStateException(String.format("Multiple BooleanListEntries added to %s.ignoreCustom", ModConfig.CollisionConfig.class.getSimpleName())); })
                            .orElseThrow(() -> new IllegalStateException(String.format("No BooleanListEntries added to %s.ignoreCustom", ModConfig.CollisionConfig.class.getSimpleName())));
                    return guis;
                },
