...
public class CoreConfigLoader<T, N extends RawConfigNode> implements ConfigLoader<T> {
    ...
    private final ConfigSerializer<N> serializer;

    public CoreConfigLoader(ConfigSerializer<N> serializer, Class<T> configClass, Path configDir, String name) {
    ...
    public void write(T config) throws Exception {
        N current = serializer.serialize(config);
        @SuppressWarnings("unchecked")
        N previous = config instanceof RawConfigHolder holder ? (N) holder.getRawConfig() : null;
        N merged = previous == null ? current : merge(current, previous);
     ...
     * <p>
     * Merges recursively when both sides have a {@link ConfigSerializer#isObject(RawConfigNode) object} at the same key.
     ...
     */
    private N merge(N current, N previous) {
        N result = serializer.deepCopy(current);
        for (Map.Entry<String, N> entry : serializer.entries(previous)) {
            String key = entry.getKey();
            N previousValue = entry.getValue();
            N currentValue = serializer.get(current, key);

            if (currentValue == null) {
                serializer.add(result, key, serializer.deepCopy(previousValue));
            } else if (serializer.isObject(currentValue) && serializer.isObject(previousValue)) {
                serializer.add(result, key, merge(currentValue, previousValue));
            }
        }
        return result;
    }
